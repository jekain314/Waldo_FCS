...
namespace Waldo_FCS
{
    public class UTM2Geodetic
    {
        ...
        public void LLtoUTM(double LatRad, double LonRad,
        ...
        public void UTMtoLL(double UTMNorthing, double UTMEasting, String UTMZone,
                      ref double LatDeg, ref double LonDeg)
        {
            ...
            LonDeg = (D - (1 + 2 * T1 + C1) * D * D * D / 6 + (5 - 2 * C1 + 28 * T1 - 3 * C1 * C1 + 8 * eccPrimeSquared + 24 * T1 * T1)
                            * D * D * D * D * D / 120) / Math.Cos(phi1Rad);
            LonDeg = LongOrigin + LonDeg * Rad2Deg;
